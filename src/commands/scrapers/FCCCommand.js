...
module.exports = class FCCCommand extends Command {
  ...
  async run(msg, { challengeName }) {
    ...
    const { title, description, url } =
      findChallenge(challengeName.trim().toLowerCase()) || defaultMessage;
...
function getMajorCategory(category) {
  ...
  for (const majorCategory in githubEndpoints) {
    for (const minorCategory of githubEndpoints[majorCategory]) {
...
function findChallenge(name) {
  ...
  const challengeData = JSON.parse(
    file.readFileSync('src/commands/scrapers/data/fcc/challengeData.json')
  );

  const possibleWordResults = [];

  for (const minorCategory of challengeData) {
    const majorCategory = getMajorCategory(minorCategory.name);
    for (const challenge of minorCategory.challenges) {
      ...
      if (nameWords.some((nameWord) => title.match(nameWord))) {
        possibleWordResults.push({
          ...
          url: `${fccURI}/${majorCategory}/${minorCategory.name
            .toLowerCase()
            .replace(/\s/g, '-')}/${title.replace(/\s/g, '-')}`,
          nMatchedWords: nameWords.reduce(
            (a, b) => (titleWords.includes(b) ? a + 1 : a),
            0
          ),
        });
      }
  ...
  if (possibleWordResults.length === 1) {
    return possibleWordResults[0];
  }
  return possibleWordResults.reduce(
    (a, b) => (b.nMatchedWords > a.nMatchedWords ? b : a)
  );
}
